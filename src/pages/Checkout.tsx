...
const Checkout = () => {
  ...
  const totalPriceToPay = cartItems.reduce((t, item) => t + item.finalPrice * item.quantity, 0);
            ...
            {cartItems.map((item, index) => (
              ...
              >
                ...
                <div className="flex gap-2 md:gap-8 flex-1">
                  ...
                  <div className="flex flex-col md:flex-row items-start gap-[13px] md:gap-2 lg:gap-9 flex-1 ">
                    <p className="text-sm font-helvetocaRegular text-blue-50 text-center md:max-w-[225px] flex-wrap">
                      {item.name} ({item.label})
                    </p>

                    <div className="flex items-center gap-2 md:gap-4 lg:gap-9">
                      <select
                        className="bg-[#F2F2F2] py-2 px-3 rounded-3xl text-blue-50 text-xl shrink-0"
                        name="amount"
                        id="amount"
                        value={item.quantity}
                ...
                <div className="flex flex-col items-end gap-1 min-w-[90px]">
                  <div className="flex items-center gap-1 flex-col" key={item.label}>
                    <div className="flex items-center gap-1 md:gap-2">
                      ...
                      <div className="text-[#FFFFFF] bg-red-100 rounded-[2.11px] px-[3.87px] py-[0.7px]">
                        <span className="redDiscount text-[11.25px]">
                          -{((item.price - item.finalPrice) * item.quantity).toFixed(2)} ₾
                        </span>
                      </div>
                    </div>
        ...
        <div className="w-full md:w-72 flex flex-col gap-4">
          ...
          {!selectedPaymentMethod && (
            <div className="flex bg-white rounded-2xl p-4 shadow-sm md:hidden flex-col gap-3">
              <div className="flex justify-between items-center">
                <h2 className="text-[18px] font-medium text-[#2f4a9c]">Delivery details:</h2>
              ...
              <div className="flex justify-between text-sm text-gray-700">
                ...
                <span className="text-[#161F28]">Tbilisi, Rustaveli 1, 01212</span>
              </div>
              <div className="flex justify-between text-sm text-gray-700">
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm flex flex-col gap-2">
            ...
            <div className="flex justify-between text-sm text-gray-700">
              ...
              <span className="text-[#161F28] text-[16px]">{totalPrice.toFixed(2)} ₾</span>
            </div>
            <div className="flex justify-between text-sm text-gray-700">
          ...
          {selectedPaymentMethod && (
            <div className="bg-white rounded-2xl p-4 shadow-sm flex flex-col gap-3">
              ...
              {[
                { value: "tbc", label: "TBC Bank", img: tbc, imgClass: "size-8.5 rounded-[7px]" },
                { value: "bog", label: "Bank of Georgia", img: bog, imgClass: "" },
          ...
          {selectedPaymentMethod ? (
            <div className="flex flex-col gap-3">
              ...
              <label className="terms-toggle px-4 md:px-0">
                ...
                <p className="text-[#797979] font-normal">I agree to terms &amp; conditions</p>
              </label>
              <button
